...
execution_measures<float> SJF(std::vector<process> process_queue){
    ...
    for(int i = 0; i < process_queue.size(); i++){
        current = i;
        min = i;
        fast_foward = process_queue[i].arrival <= t;

        for(int j = i+1; j < process_queue.size(); j++){
            if(process_queue[j].arrival < process_queue[min].arrival)
                min = j;

            if(process_queue[j].arrival <= t){
                fast_foward = false;
                if(process_queue[j].duration < process_queue[current].duration){
                    current = j;
